...
class GMM:
    ...
    def predict(self,Y):
        ...
        for m,c,cl in zip(self.mu,self.cov,colours):
            #multi_normal = multivariate_normal(mean=m,cov=c)
            ax2.contour(sc_X.inverse_transform(np.sort(self.X[:,0])),sc_Y.inverse_transform(np.sort(self.X[:,1])),sc_X.inverse_transform(multivariate_gaussian(self.XY,m,c)).reshape(len(self.X),len(self.X)),colors=cl,alpha=0.6)
